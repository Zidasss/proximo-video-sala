...
export default function Home() {
  ...
  function useData(conn: DataConnection) {
    ...
    conn.on("data", item => {
      const data = item as { kind?: string; name?: string; text?: string; active?: boolean };
  ...
  function record() {
    ...
    const contain = (video: HTMLVideoElement | null, x: number, y: number, width: number, height: number) => { if (!video?.videoWidth) return; const scale = Math.min(width / video.videoWidth, height / video.videoHeight), drawWidth = video.videoWidth * scale, drawHeight = video.videoHeight * scale; context.drawImage(video, x + (width - drawWidth) / 2, y + (height - drawHeight) / 2, drawWidth, drawHeight); };
    const draw = () => { if (!recorder.current || recorder.current.state === "inactive") return; context.fillStyle = "#101210"; context.fillRect(0, 0, canvas.width, canvas.height); if (vertical) { const cameraHeight = canvas.height * tiktokTop, gap = 12, screenHeight = canvas.height - cameraHeight - gap, half = (canvas.width - gap) / 2, first = topOrder === "mine-first" ? mine.current : theirs.current, second = topOrder === "mine-first" ? theirs.current : mine.current, screenVideo = sharing ? screen.current : remoteScreen.current, cameraY = screenPosition === "top" ? screenHeight + gap : 0, screenY = screenPosition === "top" ? 0 : cameraHeight + gap; cover(first, 0, cameraY, half, cameraHeight); cover(second, half + gap, cameraY, half, cameraHeight); contain(screenVideo, 0, screenY, canvas.width, screenHeight); } else { contain(sharing ? screen.current : remoteSharing ? remoteScreen.current : mine.current, 0, 0, canvas.width, canvas.height); cover(mine.current, canvas.width * .72, canvas.height * .68, canvas.width * .25, canvas.height * .28); cover(theirs.current, 24, canvas.height * .72, canvas.width * .2, canvas.height * .23); } requestAnimationFrame(draw); };
