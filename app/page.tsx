...
export default function Home() {
  ...
  function record() {
    ...
    const draw = () => { if (!recorder.current || recorder.current.state === "inactive") return; context.fillStyle = "#101210"; context.fillRect(0, 0, canvas.width, canvas.height); if (vertical) { const top = canvas.height * tiktokTop, gap = 12, half = (canvas.width - gap) / 2, first = topOrder === "mine-first" ? mine.current : theirs.current, second = topOrder === "mine-first" ? theirs.current : mine.current; cover(first, 0, 0, half, top); cover(second, half + gap, 0, half, top); cover(sharing ? screen.current : remoteScreen.current, 0, top + gap, canvas.width, canvas.height - top - gap); } else { cover(sharing ? screen.current : remoteSharing ? remoteScreen.current : mine.current, 0, 0, canvas.width, canvas.height); cover(mine.current, canvas.width * .72, canvas.height * .68, canvas.width * .25, canvas.height * .28); cover(theirs.current, 24, canvas.height * .72, canvas.width * .2, canvas.height * .23); } requestAnimationFrame(draw); };
