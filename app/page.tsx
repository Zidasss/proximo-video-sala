...
function OfflineStudio({ onBack }: { onBack: () => void }) {
  ...
  const recorder = useRef<MediaRecorder | null>(null), chunks = useRef<Blob[]>([]), frame = useRef(0), audioContext = useRef<AudioContext | null>(null);
  useEffect(() => { void navigator.mediaDevices.enumerateDevices().then((items) => { setCams(items.filter((item) => item.kind === "videoinput")); setMics(items.filter((item) => item.kind === "audioinput")); }); return () => { cancelAnimationFrame(frame.current); audioContext.current?.close(); streams.current.forEach((stream) => stream?.getTracks().forEach((track) => track.stop())); }; }, []);
  ...
  function draw() {
    ...
    drawVideo(a, 0, 0, half, target.height); drawVideo(b, half, 0, half, target.height);
